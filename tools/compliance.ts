...
const denoJsonHasLicenseAndTags = async (workspace: string) => {
    ...
    try {
        ...
        return denoJson.license && denoJson.tags && aiTags.some(tag => denoJson.tags.includes(tag));
    } catch {
        return false;
    }
}
...
export const aiDisclosureCommand = new Command()
    .name("compliance")
    .description("Add AI disclosure to README.md and MIT license to LICENSE and deno.json files for workspaces")
    .action(async () => {
        ...
        for (const workspace of workspaces) {
            ...
            if (!await hasReadmeFile(workspace) || !await hasAiDisclosure(workspace) || !await hasSecurityDisclosure(workspace)) {
                const readmePath = join(workspace, "README.md");
                let readmeContent = "";
                try {
                ...
                if (headerIndex !== -1) {
                    ...
                    const remainingContent = newReadmeContent.substring(headerIndex + header.length);
                    ...
                    if (nextSectionIndexRelative !== -1) {
                        const nextSectionIndex = headerIndex + header.length + nextSectionIndexRelative;
                ...
                if (firstSectionIndex !== -1) {
                     ...
                     const before = newReadmeContent.substring(0, firstSectionIndex).trimEnd();
                    ...
                    newReadmeContent = newReadmeContent + separator + recommendedReadmeFooter;
                }
                ...
                if (securityHeaderIndex !== -1) {
                     console.log(`Removing existing Security Disclosure from ${readmePath}`);
                     const secBefore = newReadmeContent.substring(0, securityHeaderIndex).trimEnd();
                     
                     // Find end of this section (start of next or EOF)
                     const secRemaining = newReadmeContent.substring(securityHeaderIndex + securityHeader.length);
                     const nextSecRelative = secRemaining.indexOf("\n## ");
                     
                     let secAfter = "";
                     if (nextSecRelative !== -1) {
                         const nextSecIndex = securityHeaderIndex + securityHeader.length + nextSecRelative;
                ...
                console.log(`Added AI disclosure and Security statement to ${readmePath}`);
            }
